...
- 遊び要素（**発見スタンプ・実績バッジ**）の記録は **localStorage のみ**。サーバーへは一切送信せず、保存が使えないブラウザではスタンプ表示を省くだけで他の機能は通常どおり動きます。
...
- 作品一覧では **絞り込み条件がURLに反映**され（`?q=&year=&sort=`）、リンク共有・再読み込みで復元できます。
- **発見記録・実績は「発見記録・実績を消す」ボタン**（作品一覧・About）でいつでも削除できます。未発見作品をシルエット表示にする任意設定もあります。
