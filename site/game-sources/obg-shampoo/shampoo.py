...
def run_game():
    ...
    try:
        bg_path = "asset/sounds/backmusic.mp3"
        if not os.path.isfile(bg_path):
            bg_path = "asset/sounds/5.mp3"
    ...
    try:
        p = "asset/sounds/7.mp3"  # ボーナスタイム開始時
        if os.path.isfile(p):
    ...
    try:
        p = "asset/sounds/9.mp3"  # ボーナスタイム中の連打
        if os.path.isfile(p):
